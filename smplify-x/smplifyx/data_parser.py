...
class OpenPose(Dataset):
    ...
    def read_item(self, img_path):
        ...
        # If the frame filename was produced by OpenPose with overlay,
        # drop known overlay suffixes so it matches the JSON base name.
        overlay_suffixes = ('_rendered', '_overlay', '_annotated')
        for suf in overlay_suffixes:
            if img_fn.endswith(suf):
                img_fn = img_fn[: -len(suf)]
                break
